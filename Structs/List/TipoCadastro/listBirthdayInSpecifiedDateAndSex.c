...
void listaAniversarioSexo(TipoCadastro cadastro, int dia, int mes, int ano, char sexo) {
    int found = 0;
    char *nomeSexo = retornaSexo(sexo);
    printf("\n=== Funcionario(s) nascido(s) em %02d/%02d/%d do sexo %s ===\n\n", dia, mes, ano, nomeSexo);
    for (int i = 0; i < cadastro.quant; i++) {
        if(cadastro.Funcionario[i].birthDate.day == dia &&
           cadastro.Funcionario[i].birthDate.month == mes &&
           cadastro.Funcionario[i].birthDate.year == ano &&
           cadastro.Funcionario[i].sex == sexo) {
            printf("Nome: %s\n", cadastro.Funcionario[i].name);
            printf("Data de Nascimento: %02d/%02d/%d\n", 
                cadastro.Funcionario[i].birthDate.day,
                cadastro.Funcionario[i].birthDate.month,
...
int main() {
    ...
    do {
        ...
        printf("12 - Encontra aniversariante pelo aniversario e sexo\n");
        printf("0 - Sair\n");
        ...
        switch(opcao) {
            ...
            case 12: {
                ...
                do {
                    printf("Digite a data de nascimento do funcionario(a) (dd/mm/yyyy):\n");
                    if (scanf("%d/%d/%d", &dia, &mes, &ano) != 3) {
                        printf("Formato de data invalido! Use dd/mm/yyyy\n");
                        clearBuffer();
                        continue;
                    }
                    clearBuffer();
